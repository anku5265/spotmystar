...
export default function Dashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [
        ...
        api.get('/api/admin-advanced/analytics/dashboard').catch(() => ({ data: null })),
        api.get('/api/admin-advanced/risk-flags?resolved=false').catch(() => ({ data: [] })),
        api.get('/api/admin-advanced/audit-log?limit=10').catch(() => ({ data: [] }))
      ]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white">
      ...
      {/* Fallback to basic stats if advanced analytics not available */}
      {!analytics && stats && (
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <button
