...
export default function NotificationBell({ userType, userId }) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {showDropdown && (
        <>
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[9998]" onClick={() => setShowDropdown(false)}></div>
          <div className="fixed right-4 top-20 w-96 bg-gray-800 rounded-xl shadow-2xl border border-gray-700 overflow-hidden z-[9999] animate-slide-in-right">
            <div className="p-4 bg-gradient-to-r from-blue-500/20 to-purple-500/20 border-b border-gray-700">
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => setShowDropdown(false)} className="text-gray-400 hover:text-white transition-colors">
                  <X size={20} />
                </button>
              </div>
            </div>
            ...
            <div className="max-h-96 overflow-y-auto">
              {notifications.length === 0 ? (
                ...
                notifications.map((notif) => (
                  <div
                    key={notif.id}
                    className={`p-4 border-b border-gray-700/50 hover:bg-white/5 transition-colors cursor-pointer ${!notif.is_read ? 'bg-blue-500/10' : ''}`}
