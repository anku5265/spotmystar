...
router.get('/artists/advanced', async (req, res) => {
  try {
    ...
    if (status) { where += ' AND a.status = $' + p; params.push(status); p++; }
    params.push(parseInt(limit), (parseInt(page) - 1) * parseInt(limit));
...
router.patch('/artists/:id/admin-update', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE artists SET status = COALESCE($1, status), is_verified = COALESCE($2, is_verified) WHERE id = $3 RETURNING *',
...
router.get('/analytics/dashboard', async (req, res) => {
  try {
    ...
    res.json({
      ...
      risks: { total_flags: 0, unresolved_flags: 0, critical_flags: 0, auto_generated_flags: 0 },
      activityTrends: []
    });
  } catch (error) { res.status(500).json({ message: error.message }); }
});
